...
class NewsDataSource( private val networkService: NewsApiService,
    ...
    override fun loadInitial(params: LoadInitialParams<Int>, callback: LoadInitialCallback<Int, News>) {
        ...
        compositeDisposable.add(
                networkService.fetchNews(1, params.requestedLoadSize)
                        .subscribe(
                                { response ->
                                    updateState(State.DONE)
                                    Timber.d("[NewsDataSource] data fetched from ${response.news.count()}")
    ...
    override fun loadAfter(params: LoadParams<Int>, callback: LoadCallback<Int, News>) {
        updateState(State.LOADING)
        compositeDisposable.add(
                networkService.fetchNews(params.key, params.requestedLoadSize)
                        .subscribe(
                                { response ->
                                    updateState(State.DONE)
                                    callback.onResult(response.news,
